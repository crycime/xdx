...
import { PoolPairData, Path, Pool, PoolDictionary, Swap, DisabledOptions } from './types';
import { BONE, MAX_IN_RATIO, MAX_OUT_RATIO, bmul, bdiv, bnum, calcOutGivenIn, calcInGivenOut, scale } from './bmath';
  ...
  if (swaps.length == 1) {
    ...
    if (swapType === 'swapExactIn') {
      return BigNumber.min(
        ...
        bmul(getLimitAmountSwap(poolPairDataSwap2.poolPairData, swapType), poolPairDataSwap1.sp) // we need to multiply the limit_IN of
      ...
      return BigNumber.min(
        ...
        bdiv(getLimitAmountSwap(poolPairDataSwap1.poolPairData, swapType), poolPairDataSwap2.sp) // we need to divide the limit_OUT of
...
export function getSpotPricePath(pools: PoolDictionary, path: Path, poolPairData: any): BigNumber {
  ...
  if (swaps.length == 1) {
    ...
    if (p1.balanceIn.isEqualTo(bnum(0)) || p2.balanceIn.isEqualTo(bnum(0))) {
      ...
      if (swapType === 'swapExactIn') {
        ...
        let denominator1 = bmul(BONE.minus(p1.swapFee), bmul(p1.balanceOut, p1.weightIn));
        let denominator2 = bmul(BONE.minus(p2.swapFee), bmul(p2.balanceOut, p2.weightIn));
...
export const parsePoolPairData = (p: Pool, tokenIn: string, tokenOut: string): PoolPairData => {
...
export function filterPools(
  ...
  if (disabledOptions.isOverRide) disabledTokens = disabledOptions.disabledTokens;
  ...
  allPools.forEach((pool) => {
    ...
    if (maxPools > 1) {
      ...
      if (containsTokenIn && !containsTokenOut) {
        tokenInPairedTokens = new Set([...tokenInPairedTokens, ...tokenListSet]);
        poolsTokenOne[pool.id] = pool;
      } else if (!containsTokenIn && containsTokenOut) {
        tokenOutPairedTokens = new Set([...tokenOutPairedTokens, ...tokenListSet]);
...
export function sortPoolsMostLiquid(
  ...
  for (let i = 0; i < hopTokens.length; i++) {
    ...
    mostLiquidPoolsFirstHop[i] = poolsTokenInNoTokenOut[highestNormalizedLiquidityFirstPoolId];
