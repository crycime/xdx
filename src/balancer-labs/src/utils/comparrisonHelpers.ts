...
export function getLimitAmountSwap(poolPairData: PoolPairData, swapType: string): BigNumber {
...
export function getLimitAmountSwapPath(pools: any[], path: Path, swapType: string): BigNumber {
  let swaps = path.swaps;
  if (swaps.length == 1) {
    let swap1 = swaps[0];
    let poolSwap1 = pools[swap1.pool];
    let poolPairDataSwap1 = parsePoolPairData(poolSwap1, swap1.tokenIn, swap1.tokenOut);
    ...
    let poolPairDataSwap2 = parsePoolPairData(poolSwap2, swap2.tokenIn, swap2.tokenOut);
    ...
    if (swapType === 'swapExactIn') {
      return BigNumber.min(
        ...
        bmul(getLimitAmountSwap(poolPairDataSwap2, swapType), getSpotPrice(poolPairDataSwap1)) // we need to multiply the limit_IN of
      ...
      return BigNumber.min(
        ...
        bdiv(getLimitAmountSwap(poolPairDataSwap1, swapType), getSpotPrice(poolPairDataSwap2)) // we need to divide the limit_OUT of
...
export function getSpotPricePath(pools: any[], path: Path): BigNumber {
  ...
  if (swaps.length == 1) {
    ...
    return bmul(getSpotPrice(poolPairDataSwap1), getSpotPrice(poolPairDataSwap2));
  } else {
    throw new Error('Path with more than 2 swaps not supported');
  }
}
...
export function getSlippageLinearizedSpotPriceAfterSwapPath(pools: any[], path: Path, swapType: string): BigNumber {
  let swaps = path.swaps;
  if (swaps.length == 1) {
    let swap1 = swaps[0];
    let poolSwap1 = pools[swap1.pool];
    ...
    if (p1.balanceIn.isEqualTo(bnum(0)) || p2.balanceIn.isEqualTo(bnum(0))) {
      ...
      if (swapType === 'swapExactIn') {
        let denominator = bmul(bmul(p1.balanceIn, p2.balanceIn), bmul(p1.weightOut, p2.weightOut));
        return bdiv(numerator, denominator);
      } else {
        let denominator = bmul(
          bmul(BONE.minus(p1.swapFee), BONE.minus(p2.swapFee)),
          bmul(bmul(p1.balanceOut, p2.balanceOut), bmul(p1.weightIn, p2.weightIn))
        );
        return bdiv(numerator, denominator);
      }
    }
...
export function getSlippageLinearizedSpotPriceAfterSwap(poolPairData: PoolPairData, swapType: string): BigNumber {
  let { weightIn, weightOut, balanceIn, balanceOut, swapFee } = poolPairData;
  if (swapType === 'swapExactIn') {
    if (balanceIn.isEqualTo(bnum(0))) {
      return bnum(0);
    } else {
      return bdiv(bmul(BONE.minus(swapFee), bdiv(weightIn, weightOut)).plus(BONE), balanceIn);
    }
  } else {
    if (balanceOut.isEqualTo(bnum(0))) {
      return bnum(0);
    } else {
      return bdiv(bdiv(weightOut, bmul(BONE.minus(swapFee), weightIn)).plus(BONE), balanceOut);
    }
  }
}

export function getReturnAmountSwapPath(pools: any[], path: Path, swapType: string, amount: BigNumber): BigNumber {
  let swaps = path.swaps;
  if (swaps.length == 1) {
    let swap1 = swaps[0];
    let poolSwap1 = pools[swap1.pool];
...
export function getReturnAmountSwap(
  ...
  if (swapType === 'swapExactIn') {
    if (balanceIn.isEqualTo(bnum(0))) {
      ...
      returnAmount = calcOutGivenIn(balanceIn, weightIn, balanceOut, weightOut, amount, swapFee);
    ...
    if (balanceOut.isEqualTo(bnum(0))) {
      ...
      returnAmount = calcInGivenOut(balanceIn, weightIn, balanceOut, weightOut, amount, swapFee);
      // Update balances of tokenIn and tokenOut
...
export function updateTokenBalanceForPool(pool: any, token: string, balance: BigNumber): any {
  // console.log("pool")
  // console.log(pool)
  // console.log("token")
  // console.log(token)
...
export async function getMultihopPoolsWithTokens(tokenIn, tokenOut) {
  ...
  const poolsTokenInNoTokenOut = filterPoolsWithoutToken(poolsTokenIn, tokenOut);
  // console.log("poolsTokenInNoTokenOut");
  // console.log(poolsTokenInNoTokenOut);

  const tokenInHopTokens = getTokensPairedToTokenWithinPools(poolsTokenInNoTokenOut, tokenIn);
  ...
  const poolsTokenOutNoTokenIn = filterPoolsWithoutToken(poolsTokenOut, tokenIn);
  // console.log(poolsTokenOutNoTokenIn);
  // console.log("poolsTokenOutNoTokenIn");

  // console.log(poolsTokenOutNoTokenIn[poolsTokenOutNoTokenIn.length-1].tokens);

  const tokenOutHopTokens = getTokensPairedToTokenWithinPools(poolsTokenOutNoTokenIn, tokenOut);
  ...
  const hopTokensSet = new Set([...Array.from(tokenInHopTokens)].filter((i) => tokenOutHopTokens.has(i)));
  ...
  for (let i = 0; i < hopTokens.length; i++) {
    ...
    for (let k in poolsTokenInNoTokenOut) {
      ...
      for (let j = 0; j < poolsTokenInNoTokenOut[k].tokensList.length; j++) {
        if (poolsTokenInNoTokenOut[k].tokensList[j].toLowerCase() == hopTokens[i]) {
      ...
      if (found) {
        let normalizedLiquidity = getNormalizedLiquidity(
          parsePoolPairData(poolsTokenInNoTokenOut[k], tokenIn, hopTokens[i].toString())
        );
    ...
    mostLiquidPoolsFirstHop[i] = poolsTokenInNoTokenOut[highestNormalizedLiquidityPoolId];
  ...
  for (let i = 0; i < hopTokens.length; i++) {
    ...
    for (let k in poolsTokenOutNoTokenIn) {
      ...
      for (let j = 0; j < poolsTokenOutNoTokenIn[k].tokensList.length; j++) {
        if (poolsTokenOutNoTokenIn[k].tokensList[j].toLowerCase() == hopTokens[i]) {
          found = true;
          break;
        }
      }
      ...
      if (found) {
        let normalizedLiquidity = getNormalizedLiquidity(
          parsePoolPairData(poolsTokenOutNoTokenIn[k], hopTokens[i].toString(), tokenOut)
        );
    ...
    mostLiquidPoolsSecondHop[i] = poolsTokenOutNoTokenIn[highestNormalizedLiquidityPoolId];
    // console.log(highestNormalizedLiquidity)
...
export const parsePoolPairData = (p, tokenIn: string, tokenOut: string): PoolPairData => {
  // console.log("Pool")
  // console.log(p)
  // console.log("tokenIn")
  // console.log(tokenIn)
...
function getTokensPairedToTokenWithinPools(pools, token) {
  ...
  for (let i in pools) {
    ...
    for (let k = 0; k < pools[i].tokensList.length; k++) {
      if (
        ethers.utils.getAddress(pools[i].tokensList[k]) != ethers.utils.getAddress(token) &&
        pools[i].tokens.find(
          (t) => ethers.utils.getAddress(t.address) === ethers.utils.getAddress(pools[i].tokensList[k])
        ).balance != 0
      ) {
