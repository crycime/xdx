...
export class BinanceBase extends BasicClient {
        ...
        if (this.batchTickers) {
            ...
            this._wss.send(
                JSON.stringify({
                    ...
                    params: this.bookTicker
                        ? [`${remote_id.toLowerCase()}@bookTicker`]
                        : [`${remote_id.toLowerCase()}@ticker`],
                    id: ++this._messageId,
                }),
            );
        }
    ...
    protected _sendUnsubTicker(remote_id: string) {
        if (this.batchTickers) {
            ...
            this._wss.send(
                JSON.stringify({
                    method: "UNSUBSCRIBE",
                    params: [`${remote_id.toUpperCase()}@ticker`],
                    id: ++this._messageId,
                }),
            );
        }
    ...
    protected _onMessage(raw: string) {
        ...
        if (msg.stream === "!ticker@arr") {
            ...
            const market = this._tickerSubs.get(msg.data.s);
