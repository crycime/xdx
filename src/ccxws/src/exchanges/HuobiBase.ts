    ...
    protected _onMessage(raw: Buffer) {
        zlib.unzip(raw, (err, resp) => {
            ...
            if (msgs.ch.endsWith(".ticker")) {
                ...
                if (msgs.tick.ask) {
                    ticker.ask = msgs.tick.ask;
                    ticker.askVolume = msgs.tick.askSize;
                    ticker.bid = msgs.tick.bid;
                    ticker.bidVolume = msgs.tick.bidSize;
                }
                // console.log("xxxx:", msgs);
            ...
            //     const ticker = this._constructTicker(msgs.tick, market);
            //     if (msgs.tick.ask) {
            //         ticker.ask = msgs.tick.ask[0];
            //         ticker.askVolume = msgs.tick.ask[1];
            //         ticker.bid = msgs.tick.bid[0];
            //         ticker.bidVolume = msgs.tick.bid[1];
            //     }
            //     // console.log("xxxx:", msgs);
            //     this.emit("ticker", ticker, market);
            //     return;
