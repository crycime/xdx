...
import { BaseClient } from '../ws-client';
import {
  BalanceUpdate,
...
type BitfinexWalletUpdateMessage = [0, BitfinexWalletUpdateMessageCommands, BitfinexWalletUpdateMessageContent];
type BitfinexPositionMessage = [0, BitfinexPositionMessageCommands, BitfinexPositionMessageContent];
type BitfinexPositionUpdateMessage = [0, BitfinexPositionUpdateMessageCommands, BitfinexPositionMessageContent];
...
const isBitfinexWalletUpdateMessage = (message: BitfinexMessage): message is BitfinexWalletUpdateMessage => {
  return Object.values(BitfinexWalletUpdateMessageCommands).includes((message as BitfinexWalletUpdateMessage)[1]);
};
...
const isBitfinexPositionUpdateMessage = (message: BitfinexMessage): message is BitfinexPositionUpdateMessage => {
  return Object.values(BitfinexPositionUpdateMessageCommands).includes((message as BitfinexPositionUpdateMessage)[1]);
};
...
export class bitfinex extends BaseClient {
  ...
  private parseBalance = (message: BitfinexWalletUpdateMessageContent): BalanceUpdate | undefined => {
    ...
    const used = !total.eq(free) && message[3] === 0 ? new Decimal(total).minus(free) : new Decimal(message[3]);
