...
describe('BatchRelayerLibrary', function () {
  ...
  it('sender can approve relayer, swap and join', async () => {
    ...
    // Swap USDC for DAI
    const placeOrderCalldata = library.interface.encodeFunctionData('swap', [
      {
    ...
    await relayer.connect(sender).multicall([getApprovalCalldata(deadline), placeOrderCalldata, joinCalldata]);
