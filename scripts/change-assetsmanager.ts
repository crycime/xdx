...
import { getOwnerPrivateKey, CONTRACT_ARBITRAGE_ADDRESS, ASSETS_MANAGER_ADDRESS } from '../.privatekey';
...
let main = async () => {
  ...
  let address = ASSETS_MANAGER_ADDRESS;
  gasLimit = (await instanceTrendingCall.estimateGas.transferAssetsManager(address)).mul(2);
  logger.debug('eth balance:', instanceTrendingCall.address, ethBalance.toString());
  let ret1 = await instanceTrendingCall.transferAssetsManager(address, {
    // gasPrice: gasPrice,
