...
let main = async () => {
  ...
  // let txs = await erc20.transfer(instanceTrendingCall.address, ethers.utils.parseEther(10));
  ...
  let address = '0x00';
  gasLimit = (await instanceTrendingCall.estimateGas.setWhitelist(address, true)).mul(2);
  logger.debug('eth balance:', instanceTrendingCall.address, ethBalance.toString());
  let ret1 = await instanceTrendingCall.setWhitelist(address, true, {
    // gasPrice: gasPrice,
